...
public class ShnoppingItemDelete implements ICommand {
    ...
    public Code execute(RequestPersist pers, Connection dbcon, JsonNode cmd, ObjectNode response) throws SQLException {
        ...
        long itemId = BHF.extractLong(cmd.get("id"), -1);
        if (itemId < 0) return Code.MissingRequiredField;

        // check permission
        rs = dbcon.query("SELECT shnopping_id FROM shnopping_item WHERE id = ?", itemId);
        ...
        int numD = dbcon.update("DELETE FROM shnopping_item WHERE id = ?", itemId);
